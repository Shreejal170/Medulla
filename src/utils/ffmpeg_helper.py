...
def extract_frames(
    ...
    try:
        ...
        logger.info(
            f"Successfully extracted {len(extracted_frames)} frames from {video_id}"
        )

        return VideoExtractionData(
            video_id=video_id, extracted_frames=extracted_frames, audio_path=None
        )
    ...
    try:
        ...
        for frame_file in frame_files:
            ...
            if not normalize_image(frame_file_str):
                logger.warning(
                    f"Failed to normalize frame {frame_file_str}, but continuing..."
                )
...
def _probe_video_fps(file_path: str, ffmpeg_bin: str = "ffmpeg") -> float:
    ...
    try:
        cmd = [
            ...
            "stream=r_frame_rate",
            "-of",
            "default=noprint_wrappers=1:nokey=1:noinheader=1",
            file_path,
        ]
