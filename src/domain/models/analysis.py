...
class ExtractedFrame(BaseModel):
    ...
    frame_id: Annotated[str, Field(description="The unique identifier for the extracted frame.", examples=['frame_0001'])]
...
class VideoExtractionData(BaseModel):
    ...
    extracted_frames: Annotated[List[ExtractedFrame], Field(description="A list of frames extracted from the video for analysis.",examples=[[{"frame_id": "frame_0001", "frame_file_path": "/path/to/frame1.jpg", "timestamp_sec": 0.033}]])]
...
class FrameAnalysis(BaseModel):
    ...
    frame_id: Annotated[str, Field(description="The unique identifier for the analyzed frame.", examples=['frame_0001'])]
...
class VideoAnalysisResult(BaseModel):
    ...
    frame_analyses: Annotated[List[FrameAnalysis], Field(description="A list of analysis results for each extracted frame in the video.", examples=[[{"frame_id": "frame_0001", "is_authentic": True, "confidence_score": 0.75, "synthesis_artifacts": ["artifact1", "artifact2"]}]])]
